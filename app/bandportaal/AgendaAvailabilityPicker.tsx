...
export function AgendaAvailabilityPicker() {
  ...
  async function blockSelectedDates() {
    ...
    const rows = selectedDates.map((date) => ({ user_id: userId, date, status: "unavailable" as const, private_note: null }));
    ...
    <aside className="portal-availability-selection" aria-live="polite">
      <div className="portal-availability-selection-inner">
        ...
        <div className="portal-availability-selection-actions">
          <button type="button" className="portal-primary" disabled={saving || selectedDates.length === 0} onClick={() => void blockSelectedDates()}>{saving ? "Datums blokkeren…" : "Geselecteerde datums blokkeren"}</button>
